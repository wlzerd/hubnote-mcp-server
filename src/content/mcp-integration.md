...
Cursor 는 Claude Desktop 과 같은 MCP 표준을 지원합니다. Settings → Cursor Settings → MCP 패널에서 같은 JSON 형식으로 추가하거나, `~/.cursor/mcp.json` 에 직접 입력.
...
## Codex CLI / IDE extension 셋업

OpenAI 의 Codex (CLI 와 IDE 확장 모두) 는 **TOML 형식** 의 별도 설정 파일을 사용합니다. CLI / IDE 가 같은 설정을 공유합니다.

### 옵션 1 — CLI 명령 한 줄 (권장)

```bash
codex mcp add hubnote \
  --env HUBNOTE_API_KEY=hbn_여기에본인키 \
  -- npx -y @hubnote-mcp/server
```

이 명령이 자동으로 `~/.codex/config.toml` 의 `[mcp_servers.hubnote]` 섹션을 추가/갱신합니다.

### 옵션 2 — `~/.codex/config.toml` 직접 편집
...
저장 후 Codex 재시작 (CLI 의 경우 새 세션 시작 시 자동 반영).

### 프로젝트 단위 등록

특정 프로젝트에서만 활성화하려면 그 프로젝트 root 에 `.codex/config.toml` 작성. Codex 가 신뢰하는 프로젝트로 등록된 경우만 적용됩니다.

## ChatGPT (현재 미지원)

ChatGPT desktop / web (Settings → Connectors) 는 **Remote MCP server (HTTPS URL)** 만 지원하며 stdio 형식의 npm 패키지를 직접 받지 않습니다.

hubnote MCP 는 1차 스코프에서 **stdio only** 로 ship 되었으므로 (D2), 현재 ChatGPT 에서 직접 셋업할 수 없습니다.

후속 사이클에서 Remote MCP server (`https://notion.discof.com/mcp` + OAuth 2.1) 를 추가하면 ChatGPT 의 Connectors 메뉴에서 URL 한 줄로 등록 가능해질 예정입니다. 그때까지는 Claude Desktop / Cursor / Codex 사용을 권장합니다.

## Claude.ai (web) 도 같은 한계

Claude 의 웹 / 모바일 (claude.ai) 의 Connectors 도 Remote MCP server 만 지원합니다. 현재는 **Claude Desktop** 데스크톱 앱을 사용해주세요.
...
### Yjs 협업 페이지의 즉시 반영 (해결됨 — 2026-05-10)

`update_page` 도구가 BlockNote 페이지를 수정하면 Yjs 협업 활성 페이지에서도 **즉시 라이브 반영**됩니다.

내부 흐름:
- API 가 호출자가 API 키임을 감지하면 yjs server 의 broadcast endpoint 를 호출
- yjs server 가 새 본문을 BlockNote → Y.Doc 으로 변환해 메모리 Y.Doc 에 적용
- Hocuspocus 의 sync 프로토콜이 모든 connected 클라이언트에 자동 broadcast
- 사용자 브라우저는 cursor / 동시 입력 끊김 없이 머지된 결과 반영

**다중 멤버 동시 시청**: 다른 멤버가 같은 페이지를 보고 있어도 awareness cursor 유지 + 본인의 진행 중 입력 (IME 포함) 손실 없이 변경이 머지됩니다 (CRDT 표준 동작).

**Best-effort 정책**: yjs server 가 일시적으로 응답 안 해도 `page_contents` 는 갱신됨 — 다음 페이지 새로고침에서 변경 반영.

**손실되는 시각 요소**: round-trip 시 BlockNote 의 일부 props (예: callout 색상, 표 헤더 스타일) 가 사라질 수 있음. 본문 / 헤딩 / 리스트 / 표 / 체크박스 / 코드블록 / 멘션 / divider 는 정확.
...
- 패키지 소스: https://github.com/wlzerd/hubnote-mcp-server
